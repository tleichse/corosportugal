// Verbatim PT copy from the project spec (§4), used as a fallback so the
// page renders correctly before a Sanity project exists / before the
// paginaInstitucional singleton has been created in Studio. Once Sanity is
// wired up, editors should correct typos there — not here. Keep this in
// sync with studio/schemaTypes/paginaInstitucional.ts's initialValue if the
// official text changes.
export const institutionalFallbackPt = {
  quemSomos: [
...
// No English fallback is provided on purpose: the spec (§4) explicitly
// says the EN translation must be written and approved by the Direção,
// not machine-generated. The EN page shows a "pending translation" notice
// instead of fabricated copy until Sanity (or this file) has real text.
